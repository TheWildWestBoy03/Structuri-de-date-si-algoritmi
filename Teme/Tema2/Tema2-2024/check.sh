...
function valgrind_test() {
    ...
    echo $test_no >> time.err
    # Get the input and ref files for the current test
    timeout=${VALGRIND_TIMEOUT_TIME[$test_no]}

    (time timeout $timeout valgrind --track-origins=yes --leak-check=full --show-leak-kinds=all -q --error-exitcode=1 ./$EXEC $input_path >$out_path) 2>> time.err
